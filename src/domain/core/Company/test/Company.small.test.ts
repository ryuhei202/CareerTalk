...
describe("Company", () => {
	describe("Companyを生成する", () => {
		...
		test("不正なCompanyIdでCompanyを生成しようとするとエラーが発生する", () => {
			expect(() => Company.create({ ...companyDummyParams, id: 0 })).toThrow(
				ZodError,
			);
		});

		test("不正なCompanyNameでCompanyを生成しようとするとエラーが発生する", () => {
			expect(() => Company.create({ ...companyDummyParams, name: "" })).toThrow(
				ZodError,
			);

			expect(() =>
				Company.create({ ...companyDummyParams, name: "a".repeat(101) }),
			).toThrow(ZodError);
		});

		test("不正なCodeでCompanyを生成しようとするとエラーが発生する", () => {
			expect(() => Company.create({ ...companyDummyParams, code: "" })).toThrow(
				ZodError,
			);

			expect(() =>
				Company.create({ ...companyDummyParams, code: "あいうえおかきく" }),
			).toThrow(ZodError);
		});
	});
});
